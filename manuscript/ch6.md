...
foo( arr.slice() );			// 哈！一个拷贝！

console.log( arr[0] );		// 1
```

当然，仅在 `foo` 没有跳过它的形式参数而通过自由变量的词法引用来引用我们同一个 `arr` 时，这种断言才能够成立！
...
你们中的一些人可能会浮现出这样的描述，“一个不能改变的值”，“一个不能被改变的变量”，等等。这些大约都是正确答案的邻居，但不是十分正确的门牌号码。一个常量的准确定义应当是：一个不能被重新赋值的变量。
...
为了证明我的判断，让我们检视一下实际情况。`const` 会创建一个块作用域变量，意味着这个变量仅会存在于这个局部化的块中：
...
如果你看看 `let x = 2` 后面的几行代码；你很容易就能知道 `x` 实际上没有被重新赋值。与使用什么 `const` 声明来表示“不会给它重新赋值”相比，这对我来说是一种 **强烈得多的信号** —— 实际上没有给它重新赋值。
...
那么同样，如果这样的操作将频繁发生，或者特别地发生在你应用程序的 *关键路径* 上，那么性能 —— 性能与内容两者! —— 就完全是一个需要关心的问题了。
