...
在 `ch11-code/stock-ticker.js` 中，我们在 `stockTickerUI` 对象上将 UI（DOM 副作用）的行为定义为方法。我们还定义了各种帮助函数，包括 `getElemAttr(..)`、`stripPrefix(..)` 以及其他一些。最后，我们 `subscribe(..)` 两个向我们提供格式化数据的 observable 来渲染 DOM。
...
在 `transformObservable(..)` 上有一个微妙的地方需要注意：因为在一个 observable 上进行 `map(..)` 返回一个新的 observable，所以它看起来是纯粹的。但从技术上讲，在它的底层，`obsv` 的内部状态被改变为连接到 `map(..)` 返回的新 observable。这种副作用没什么大不了的，而且不会损害我们代码的可读性，但是无论副作用在什么地方你都能发现它们非常重要，而不是在你遇到 bug 时被它们吓一跳。
...
我们从事件名称的数组开始 （`["stock","stock-update"]`），将这个列表 `map(..)`（参见第八章）为一个包含两个 observable 的列表，然后将这个列表 `zip(..)`（参见第八章）为一个 observable 映射函数的列表；这个映射生成了一个元组的列表，就像 `[ observable, mapperFn ]`。最后，我们使用 `transformObservable(..)` 来 `map(..)` 这个元组的列表，并使用 `spreadArgs(..)`（参见第三章）来把每个元组分散为独立的参数。
...
和之前一样，`reverseArgs( partial )` 是 `partialRight(..)` 性能优化后的替代品。但是这次，`partial(..)` 是我们要使用的映射函数。为此，我们需要 `curry(..)` 它以便提前制定第二个参数 `ticker`；当每个 UI 方法之后被映射时，它会使用 `ticker` 对这个函数进行局部应用。现在，这两个在结果数组中的双重局部应用函数可以用于订阅 observable 了。
