...
`formatPrice(..)`, `formatChange(..)`, and `processNewStock(..)` all use `pipe(..)` to compose some operations left-to-right (see [Chapter 4](ch4.md)).
...
`unboundMethod("subscribe")` will already be curried, so we `uncurry(..)` it (see [Chapter 3](ch3.md/#no-curry-for-me-please)).
...
As we draw the main text of this book to a close, I want to remind you of this readability curve that I [shared back in Chapter 1](ch1.md/#readability):
