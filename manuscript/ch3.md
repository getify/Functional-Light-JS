...
It won't seem clear yet why these occasions arise, but you will see them often. Essentially, `spreadArgs(..)` allows us to define functions that `accept` multiple values via an array as the parameter, but still have those multiple values treated independently as inputs to another function.
