...
As a general notion, if our data object doesn't have a property, we shouldn't update that corresponding element. So, we need to `filterOut(..)` any tuples where the value (in this case, in the second position) is `undefined`:
...
The result after this filtering is a list of tuples (like `[ <span>, ".." ]`) for DOM content updating, which we assign to `elemsValsTuples`.

**Note:** The `updateValueMissing(..)` predicate is not wrapped with `spreadArgs(..)`, to avoid an extraneous function wrapping. However, we still want to spread out the single array argument as two individual named parameters; we use parameter array-destructuring in the function declaration (`function updateValueMissing([infoChildElem,val]){ ..`).
