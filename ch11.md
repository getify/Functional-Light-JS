...
**Note:** The `updateValueMissing(..)` predicate is not wrapped with `spreadArgs(..)`, to avoid an extraneous function wrapping. However, we still want to spread out the single array argument as two individual named parameters; we use parameter array-destructuring in the function declaration (`function updateValueMissing([infoChildElem,val]){ ..`) -- see Chapter 2.
