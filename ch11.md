...
For creating our observables from our event emitter, we're going to want a helper that's a curried (see Chapter 3) standalone of RxJS's `Rx.Observable.fromEvent(..)`:
...
We start with the array of event names (`["stock","stock-update"]`), then `map(..)` (see Chapter 8) that list to a list of two observables, and `zip(..)` (see Chapter 8) that list to a list of observable-mapper functions; this mapping produces a list of tuples like `[ observable, mapperFn ]`. Finally, we `map(..)` that list of tuples with `transformObservable(..)`, spreading out each tuple to individual arguments using `spreadArgs(..)`.
...
First, we use `listify(..)` to ensure we have a list of elements (even if it's only a single item in length). Recall `flatMap(..)` from Chapter 8, which maps a list and then flattens a list-of-lists into a shallower list. Our mapping function here maps from an element to its `childNodes` list, which we make into a real array (instead of a live NodeList) with `Array.from(..)`.
...
As a general notion, if our data object doesn't have a property, we shouldn't update that corresponding element. So, we need to `filterOut(..)` any tuples where the value (in this case, the second position) is `undefined`. The result after filtering is a list of tuples (like `[ <span>, ".." ]`) for DOM content updating, called `elemsValsTuples`.
...
We iterate this `elemsValsTuples` list with `each(..)` (more side effects!). Instead of using `pipe(..)` as elsewhere, this composition uses `compose(..)` (see Chapter 4) to pass `setDomContent(..)` into `spreadArgs(..)`, and then that function in as the iterator-function to `each(..)`. Each tuple is spread out as the arguments to `setDOMContent(..)`, which then updates the DOM element accordingly.
...
First, we notice that those methods each expect `tickerElem` as first parameter. Let's make a list (`stockTickerUIMethodsWithDOMContext`) that encapsulates the ticker widget's DOM element with each of these two methods, via partial application:
...
`unboundMethod("subscribe")` is automatically curried, so we `uncurry(..)` it (see Chapter 3), then adapt it with `spreadArgs(..)` so it'll spread out a single tuple array as its two arguments.
